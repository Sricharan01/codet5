    ...
    if args.model_type == 'task_prefix':
        def tokenize_function(examples):
            ...
            with tokenizer.as_target_tokenizer():
                code_output_encodings = tokenizer(examples['code'], max_length=32, truncation=True)
                docstring_output_encodings = tokenizer(examples['docstring'], max_length=32, truncation=True)

            model_inputs['labels'] = code_output_encodings['input_ids']
            model_inputs['aux_labels'] = docstring_output_encodings['input_ids']

            return model_inputs

    tokenized_datasets = datasets.map(
        tokenize_function,
        remove_columns=['repo', 'path', 'func_name', 'original_string', 'language', 'code', 'code_tokens', 'docstring', 'docstring_tokens', 'sha', 'url', 'partition'],
        batched=True
    )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--from_pretrained', type=str, default='google/t5-v1_1-small')
    ...
    parser.add_argument('--output_docstring', action='store_true')
